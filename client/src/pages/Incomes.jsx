...
export default function Incomes() {
  ...
  const load = async ()=>{
    ...
    setCats(c.filter(x=>x.type==="INCOME"));
    setItems(e.items);
    if (!form.categoryId && c.length) setForm(f=>({...f, categoryId: c.find(x=>x.type==="INCOME")?.id || ""}));
  };
  useEffect(()=>{ load(); },[]);

  const submit = async (e)=>{
    e.preventDefault();
    await api.post("/api/incomes", { ...form, amount: Number(form.amount), date: new Date(form.date) });
    setForm(f=>({ ...f, amount:"", note:"" }));
    load();
  };
  const del = async (id)=>{ await api.delete(`/api/incomes/${id}`); load(); };

  return (
    <div className="space-y-6 bg-gray-900 min-h-screen text-white p-6">
      <h1 className="text-2xl font-semibold text-white">Incomes</h1>
            ...
            <tbody className="divide-y divide-gray-700">
              {items.map(x=>(
                <tr key={x.id} className="hover:bg-gray-750 transition-colors duration-150">
                  ...
                  <td className="p-4 text-green-400 font-medium">{Number(x.amount).toLocaleString()}</td>
